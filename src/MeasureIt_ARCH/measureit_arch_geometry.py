...
def draw_axisDimension(context, myobj, measureGen, dim, mat, svg=None, dxf=None):
    ...
    with OpenGL_Settings(dimProps):
        ...
        leadEndA = Vector(basePoint) + offsetDistance + cap_extension(offsetDistance,
                                                                      dimProps.endcapSize, dimProps.endcapArrowAngle)
...
def draw_areaDimension(context, myobj, DimGen, dim, mat, svg=None, dxf=None):
    ...
    with OpenGL_Settings(dimProps):
        ...
        for textField in dim.textFields:
            ...
            textField['textcard'] = generate_text_card(
                context, textField, dimProps, basePoint=origin, xDir=vecX, yDir=vecY.normalized(), cardIdx=idx)
        ...
        if sceneProps.is_vector_draw:
            svg_dim = svg.add(svg.g(id=dim.name))
            svg_shaders.svg_poly_fill_shader(dim, polyfillCoords, (0, 0, 0, 0), svg,
                                             line_color=rgb, lineWeight=lineWeight, itemProps=dimProps)
...
def draw_line_group(context, myobj, lineGen, mat, svg=None, dxf=None, is_instance_draw=False):
    ...
    for lineGroup in lineGen.line_groups:
        ...
        if lineGroup.endcapA != 'NONE':
            dot, fill = draw_annotation_endcaps(lineGroup, lineGroup.endcapA, mat @
                                                Vector(coords[0]), mat @ Vector(coords[-1]), rgb, lineGroup.endcapSize)
            dotcoords.append(dot)
            filledcoords.append(fill)

        if lineGroup.endcapB != 'NONE':
            dot, fill = draw_annotation_endcaps(lineGroup, lineGroup.endcapB, mat @
                                                Vector(coords[-1]), mat @ Vector(coords[-2]), rgb, lineGroup.endcapSize)
            dotcoords.append(dot)
            filledcoords.append(fill)
        ...
        if sceneProps.is_vector_draw:
            if not lineProps.chain:
                ...
                svg_shaders.svg_poly_fill_shader(lineGroup, coords, (0, 0, 0, 0), svg, line_color=rgb,
                                                 lineWeight=lineProps.lineWeight, itemProps=lineProps, closed=False, mat=mat)
...
def draw_annotation(context, myobj, annotationGen, mat, svg=None, dxf=None, instance=None):
    ...
    for annotation in annotationGen.annotations:
        ...
        if sceneProps.is_dxf_draw:
            ...
            dxf_shaders.dxf_line_shader(annotation, annotationProps, coords,
                                        annotationProps.lineWeight, rgb, dxf, myobj)
...
def draw_table(context, myobj, tableGen, mat, svg=None, dxf=None, instance=None):
    ...
    for table in tableGen.tables:
        ...
        if table.textFile.is_dirty or table.text_file_updated:
            ...
            while len(table.rows) < max_rows:
                table.rows.add()
            while len(table.rows) > max_rows:
                table.rows.remove(len(table.rows) - 1)

            while len(table.columns) < max_columns:
                table.columns.add()
            while len(table.columns) > max_columns:
                table.columns.remove(len(table.columns) - 1)
            ...
            for row_idx in range(max_rows):
                ...
                for col_idx in range(max_columns):
                    ...
                    while len(row.textFields) < max_columns:
                        row.textFields.add()
                    while len(row.textFields) > max_columns:
                        row.textFields.remove(len(row.textFields) - 1)
                    try:
        ...
        for row_idx in range(len(table.rows)):
            ...
            for col_idx in range(len(table.columns)):
                ...
                textField = row.textFields[col_idx]
                if textField.textHeight > row.height:
                    row.height = textField.textHeight
                if textField.textWidth > col.width:
                    col.width = textField.textWidth
        ...
        filled_coords = [origin, origin + i * table_width, origin - j * cell_y, origin +
                         i * table_width, origin + - j * cell_y, origin + i * table_width - j * cell_y]
...
def get_textField_boundary(context, textField):
    if textField.boundaryShape == 'NONE':
        return
    card = None
    try:
    ...
    if textField.boundaryShape == 'ROUNDED':
        ...
        for i in range(0, segments):
            ...
            coords.extend([lstart_point + left_mid, l_next_point + left_mid,
                          rstart_point + right_mid, r_next_point + right_mid])
...
def draw_lines(lineWeight, rgb, coords, offset=-0.001, pointPass=False, dashed=False,
    ...
    if invalid:
        ...
        if type(rgb) == list:
            buffer['colors'].extend(rgb)
        else:
            vboBuffer['colors'].extend([rgb] * num_coords)
        # Check for list of weight values
        if type(lineWeight) == list:
            vboBuffer['weights'].extend(lineWeight)
        else:
            vboBuffer['weights'].extend([lineWeight] * num_coords)
...
def setup_dim_text(myobj, dim, dimProps, dist, origin, distVector, offsetDistance, is_area=False):
    ...
    for textField in dim.textFields:
        set_text(textField, myobj)
        placementResults = dim_text_placement(dim, dimProps, origin, dist, distVector,
                                              offsetDistance, dimProps.endcapSize, cardIdx=idx, textField=dim.textFields[idx])
        if idx == 0:
            flipCaps = placementResults[0]
            dimLineExtension = placementResults[1]
            origin = placementResults[2]
